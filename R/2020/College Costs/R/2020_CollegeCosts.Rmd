...
##QUESTION: What are the most expensive colleges in the US by in-state total?

#2018-2019: Most Expensive 4-Year Colleges Plot
tuition_cost_clean %>% 
...
##QUESTION: Most expensive and inexpensive colleges per state?
tuition_cost_clean %>% 
  ...
  mutate(pct_change = -diff(in_state_total) / max(in_state_total)) %>% #percent change to create a ranking for plot
  arrange(desc(pct_change)) %>% 
  ungroup() %>% 
  top_n(n = 30, wt = pct_change) %>% 
