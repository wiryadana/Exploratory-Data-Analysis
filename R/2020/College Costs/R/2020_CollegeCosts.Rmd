...
foo_2 <- salary_potential %>% 
  ...
  summarise(median_early_pay = median(early_career_pay),
            median_middle_pay = median(mid_career_pay)) %>% 
  #mutate(early_middle_diff = -(median_early_pay - median_middle_pay)) %>% 
  group_by(state_name) %>% 
  arrange(state_name, (desc(median_early_pay))) %>% 
  slice(1) %>% 
  arrange(desc(median_early_pay)) %>% 
  unite(col = state_school, c(state_name, name), sep = " - ")
foo_2 

foo_2$state_school <- fct_reorder(foo_2$state_school, foo_2$median_middle_pay)

foo_2_plot <- foo_2 %>% 
pivot_longer(cols = c(median_early_pay, median_middle_pay), names_to = "pay_stage", values_to = "pay") %>% 
ggplot(aes(y = state_school))+
  ...
  labs(title = "What Schools Did the Highest Earning Alumni Attend?",
       subtitle = "The Top School Per State by Median Pay in 2019",
       x = "Pay",
       y = "",
       fill = "Career Stage",
